...
let UsersService = class UsersService {
    ...
    async create(dto) {
        ...
        if (!tenantId)
            throw new common_1.ConflictException('Tenant no detectado');
        const exists = await this.repo.findOne({ where: { email: dto.email, tenant_id: tenantId } });
        if (exists)
            throw new common_1.ConflictException('Email ya registrado en este tenant');
