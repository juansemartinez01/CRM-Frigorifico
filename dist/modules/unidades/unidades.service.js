...
let UnidadesService = class UnidadesService {
    ...
    async create(dto) {
        ...
        if (exists)
            throw new common_1.ConflictException('Ya existe una unidad con ese nombre');
    ...
    async findAll(q) {
        ...
        if (q.q)
            qb.andWhere('(u.nombre ILIKE :q OR u.simbolo ILIKE :q)', { q: `%${q.q}%` });
        qb.orderBy(`u.${q.orderBy ?? 'nombre'}`, (q.order ?? 'ASC'));
